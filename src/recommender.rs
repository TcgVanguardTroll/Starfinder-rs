...
/// Each level drills into a more specific attribute.
#[derive(Debug, Clone)]
...
impl PreferenceNode {
    pub fn pct(&self) -> f64 {
        if self.parent_count == 0 { 0.0 } else {
            self.count as f64 / self.parent_count as f64 * 100.0
        }
    }
}

/// Builds a body_type → ethnicity → hair_color preference tree.
pub fn build_preference_tree(performers: &[Performer]) -> Vec<PreferenceNode> {
    let total = performers.len();
    build_level(performers, total, 0)
}

fn build_level(performers: &[Performer], parent_count: usize, depth: usize) -> Vec<PreferenceNode> {
    // Stop at 3 levels: body_type → ethnicity → hair_color
    if depth >= 3 || performers.is_empty() {
        return vec![];
    }

    let groups = group_by_attribute(performers, depth);
...
/// Prints the tree with ASCII lines and percentages.
...
/// Follows the dominant path (highest-count child at each level) and returns
/// the labels as a search query.  Stops when confidence drops below 50%.
pub fn dominant_query_path(nodes: &[PreferenceNode]) -> Vec<String> {
    let mut path = vec![];
    let mut current = nodes;

    loop {
        let Some(best) = current.first() else { break }; // already sorted desc
...
// ── flat analysis (kept for stats display) ───────────────────────────────────
